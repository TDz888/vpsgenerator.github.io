...
export async function validateGitHubToken(token) {
  if (!isValidGitHubTokenFormat(token)) {
    return { valid: false, error: 'Token GitHub không đúng định dạng. Token phải bắt đầu bằng "github_pat_" hoặc "ghp_". Vui lòng tạo token mới tại https://github.com/settings/tokens' };
  }
...
export async function createRepository(token, name, description) {
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 422) throw new Error(`Tên repository không hợp lệ: ${err.message}`);
      if (res.status === 401) throw new Error('Token không có quyền tạo repository. Cần quyền "repo".');
